...
export class AddMatrixOrganization1796700000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'CREATE INDEX idx_personnel_tenant_id ON personnel(tenant_id)',
    );
    await queryRunner.query(
    ...
    await queryRunner.query(`CREATE TABLE personnel_assignments (
      ...
      created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now())`);
    await queryRunner.query(
      'CREATE INDEX idx_personnel_assignments_tenant_id ON personnel_assignments(tenant_id)',
    );
    await queryRunner.query(
      'CREATE INDEX idx_personnel_assignments_org_position ON personnel_assignments(organization_unit_id, position_id)',
    );
    await queryRunner.query(
      'CREATE UNIQUE INDEX uq_personnel_assignments_active_position ON personnel_assignments(personnel_id, organization_unit_id, position_id) WHERE end_date IS NULL',
    );
  }
