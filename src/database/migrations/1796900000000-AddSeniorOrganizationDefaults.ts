...
export class AddSeniorOrganizationDefaults1796900000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'UPDATE personnel_assignments SET rank = rank + 1 WHERE rank BETWEEN 1 AND 5',
    );
    await queryRunner.query(
      'ALTER TABLE personnel_assignments ALTER COLUMN rank SET DEFAULT 6',
    );
    await queryRunner.query(`
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      "DELETE FROM positions WHERE code IN ('CHU_TICH_HOI_DONG_QUAN_TRI','THANH_VIEN_HOI_DONG_QUAN_TRI','TONG_GIAM_DOC','PHO_TONG_GIAM_DOC','GIAM_DOC_KHOI') AND metadata->>'systemDefault' = 'true'",
    );
    await queryRunner.query(
      'UPDATE personnel_assignments SET rank = rank - 1 WHERE rank BETWEEN 2 AND 6',
    );
    await queryRunner.query(
